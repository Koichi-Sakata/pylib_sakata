...
plot.plot_xy(ax2, time, ErrPosUm, '-', 'r', 1.5, 1.0)
plot.plot_xy(ax2, time_pf, ErrPosUm_pf, '--', 'm', 1.5, 1.0, yrange=[-1.0, 1.0], ylabel='Error Pos [um]')
plot.plot_xy(ax3, time, ServoOutN, '-', 'r', 1.5, 1.0)
plot.plot_xy(ax3, time_pf, ServoOutN_pf, '--', 'm', 1.5, 1.0, xlabel='Time [s]', ylabel='ServoOut [N]', legend=['with NF only', 'with NF+PF'])
...
plot.plot_xy(ax1, time, ErrPosUm, '-', 'r', 1.5, 1.0)
plot.plot_xy(ax1, time_pf, ErrPosUm_pf, '--', 'm', 1.5, 1.0, yrange=[-1.0, 1.0], ylabel='Error Pos [um]', legend=['w/o PF (Exp)', 'with PF (Exp)'], loc='upper right', title='Time response')
plot.plot_xy(ax2, time, ErrPosUm, '-', 'r', 1.5, 1.0)
plot.plot_xy(ax2, time_sim, ErrPosUm_pf_sim, '--', 'm', 1.5, 1.0, yrange=[-1.0, 1.0], xlabel='Time [s]', ylabel='Error Pos [um]', legend=['w/o PF (Exp)', 'with PF (Sim)'], loc='upper right')
...
ax_mag = fig.add_subplot(211)
ax_phase = fig.add_subplot(212)
plot.plot_tffrd(ax_mag, ax_phase, Gn_frd, '-', 'r', 1.5, 1.0, title='Frequency response of open loop transfer function')
plot.plot_tffrd(ax_mag, ax_phase, G_frd, '--', 'm', 1.5, 1.0, freqrange, legend=['with NF only', 'with NF+PF'])
...
ax_phase = None
plot.plot_tffrd(ax_mag, ax_phase, Sn_frd, '-', 'r', 1.5, 1.0, title='Frequency response of sensitivity function')
plot.plot_tffrd(ax_mag, ax_phase, S_frd, '--', 'm', 1.5, 1.0, freqrange, [-60, 10], legend=['with NF only', 'with NF+PF'])
...
ax_mag = fig.add_subplot(211)
ax_phase = fig.add_subplot(212)
plot.plot_tffrd(ax_mag, ax_phase, Tn_frd, '-', 'r', 1.5, 1.0, title='Frequency response of complementary sensitivity function')
plot.plot_tffrd(ax_mag, ax_phase, T_frd, '--', 'm', 1.5, 1.0, freqrange, [-60, 10], legend=['with NF only', 'with NF+PF'])
plot.savefig(figurefolderName+'/freq_T.png')

# Nyquist
fig = plot.makefig()
ax = fig.add_subplot(111)
plot.plot_nyquist(ax, Gn_frd, '-', 'r', 1.5, 1.0, title='Nyquist diagram')
plot.plot_nyquist(ax, G_frd, '--', 'm', 1.5, 1.0, legend=['with NF only', 'with NF+PF'])

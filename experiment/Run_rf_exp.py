...
# Common parameters
figurefolderName = 'figure_rf_exp'
...
measdata_rf = meas.getdata('data/time_resp_rf.csv')
time_rf = measdata_rf.time
RefPosUm_rf = measdata_rf.value[meas.getdataindex(measdata_rf, 'RefPosUm[0]')]
ErrPosUm_rf = measdata_rf.value[meas.getdataindex(measdata_rf, 'ErrPosUm[0]')]
ServoOutN_rf = measdata_rf.value[meas.getdataindex(measdata_rf, 'ServoOutN[0]')]
# FFT
freq_fft_rf, ErrPosUm_fft_rf = fft.fft(ErrPosUm_rf[8000:72000], Ts)
...
freqRF = [10.0, 20.0, 30.0, 50.0, 60.0, 70.0, 90.0]
zetaRF = [0.002, 0.002, 0.002, 0.001, 0.001, 0.001, 0.001]
depthRF = [0.1, 0.05, 0.05, 0.1, 0.1, 0.1, 0.1]
...
print('Frequency response analysis is running...')
# Measurement with RF
G_frd = Pmeas_frd * Cz_frd * NFz_frd * (1.0+RFz_frd)
S_frd = 1/(1 + G_frd)
T_frd = 1 - S_frd

print('Simulating time response with resonant filters')
# Time response
time_sim, ErrPosUm_rf_sim = fft.frdsim(S_frd/Sn_frd, ErrPosUm, Ts)
# FFT
freq_fft_sim, ErrPosUm_fft_rf_sim = fft.fft(ErrPosUm_rf_sim[8000:72000], Ts)
...
plot.plot_xy(ax2, time_rf, ErrPosUm_rf, '--', 'r', 1.5, 1.0, yrange=[-4.0, 4.0], ylabel='Error Pos [um]')
plot.plot_xy(ax3, time, ServoOutN, '-', 'b', 1.5, 1.0)
plot.plot_xy(ax3, time_rf, ServoOutN_rf, '--', 'r', 1.5, 1.0, xlabel='Time [s]', ylabel='ServoOut [N]', legend=['w/o RF', 'with RF'])
...
plot.plot_xy(ax1, freq_fft_rf, ErrPosUm_fft_rf, '--', 'r', 1.5, 1.0, xscale='log', xrange=[1.0, 1000.0], yrange=[0.0, 1.6], xlabel='Frequency [Hz]', ylabel='Error Pos [um]', legend=['w/o RF', 'with RF'])
...
plot.plot_xy(ax1, time_rf, ErrPosUm_rf, '--', 'r', 1.5, 1.0, yrange=[-4.0, 4.0], ylabel='Error Pos [um]', legend=['w/o RF (Exp)', 'with RF (Exp)'], loc='upper right', title='Time response')
plot.plot_xy(ax2, time, ErrPosUm, '-', 'b', 1.5, 1.0)
plot.plot_xy(ax2, time_sim, ErrPosUm_rf_sim, '--', 'r', 1.5, 1.0, yrange=[-4.0, 4.0], xlabel='Time [s]', ylabel='Error Pos [um]', legend=['w/o RF (Exp)', 'with RF (Sim)'], loc='upper right')
...
ax2 = fig.add_subplot(212)
plot.plot_xy(ax1, freq_fft, ErrPosUm_fft, '-', 'b', 1.5, 1.0, title='Power spectrum density')
plot.plot_xy(ax1, freq_fft_rf, ErrPosUm_fft_rf, '--', 'r', 1.5, 1.0, xscale='log', xrange=[1.0, 1000.0], yrange=[0.0, 1.6], ylabel='Error Pos [um]', legend=['w/o RF (Exp)', 'with RF (Exp)'])
plot.plot_xy(ax2, freq_fft, ErrPosUm_fft, '-', 'b', 1.5, 1.0)
plot.plot_xy(ax2, freq_fft_sim, ErrPosUm_fft_rf_sim, '--', 'r', 1.5, 1.0, xscale='log', xrange=[1.0, 1000.0], yrange=[0.0, 1.6], xlabel='Frequency [Hz]', ylabel='Error Pos [um]', legend=['w/o RF (Exp)', 'with RF (Sim)'])
...
plot.plot_tffrd(ax_mag, ax_phase, G_frd, '--', 'r', 1.5, 1.0, freqrange, legend=['w/o RF', 'with RF'])
...
plot.plot_tffrd(ax_mag, ax_phase, S_frd, '--', 'r', 1.5, 1.0, freqrange, [-60, 10], legend=['w/o RF', 'with RF'])
...
plot.plot_tffrd(ax_mag, ax_phase, T_frd, '--', 'r', 1.5, 1.0, freqrange, [-60, 10], legend=['w/o RF', 'with RF'])
...
plot.plot_nyquist(ax, G_frd, '--', 'r', 1.5, 1.0, legend=['w/o RF', 'with RF'])
